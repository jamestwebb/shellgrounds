...
// The secret must never have a default: a committed fallback would ship in the client
// bundle and let anyone recompute every user's flags offline.
export function generateUserFlag(sessionSecret, handle, challengeId) {
  if (!sessionSecret) {
    throw new Error('generateUserFlag requires a session secret');
  }
  const hmacHex = hmacSha256(sessionSecret, `${handle.toLowerCase()}:${challengeId}`);
  const bytes = [];
...
// Generate signed session token: base64(handle:expiry:hmac)
export function createSessionToken(sessionSecret, handle, expiryHours = 72) {
  if (!sessionSecret) {
    throw new Error('createSessionToken requires a session secret');
  }
  const expiry = Date.now() + (expiryHours * 60 * 60 * 1000);
  const sig = hmacSha256(sessionSecret, `${handle}:${expiry}`);
  const payload = `${handle}:${expiry}:${sig}`;
