...
// Simple MD5 computation for honest forensic hashing
...
// HMAC-SHA256 implementation (works in Node and browser)
export function hmacSha256(secret, message) {
  ...
  const innerStr = String.fromCharCode.apply(null, iKeyPad) + message;
  const innerHashHex = sha256Sync(innerStr);
  ...
  const outerStr = String.fromCharCode.apply(null, oKeyPad) + String.fromCharCode.apply(null, innerHashBytes);
...
export function generateUserFlag(sessionSecret, handle, challengeId, packId = '') {
  ...
  const scope = packId ? `${packId}:${handle.toLowerCase()}:${challengeId}` : `${handle.toLowerCase()}:${challengeId}`;
  const hmacHex = hmacSha256(sessionSecret, scope);
  const bytes = [];
  for (let i = 0; i < hmacHex.length; i += 2) {
    bytes.push(parseInt(hmacHex.substr(i, 2), 16));
  }
...
export function createSessionToken(sessionSecret, handle, expiryOrPackId = 72, packId = '') {
  if (!sessionSecret) {
    throw new Error('createSessionToken requires a session secret');
  }
  let expiryHours = 72;
  let targetPackId = packId;
  if (typeof expiryOrPackId === 'string') {
    targetPackId = expiryOrPackId;
  } else if (typeof expiryOrPackId === 'number') {
    expiryHours = expiryOrPackId;
  }

  const expiry = Date.now() + (expiryHours * 60 * 60 * 1000);
  const payloadToSign = targetPackId ? `${handle}:${targetPackId}:${expiry}` : `${handle}:${expiry}`;
  const sig = hmacSha256(sessionSecret, payloadToSign);
  const payload = targetPackId ? `${handle}:${targetPackId}:${expiry}:${sig}` : `${handle}:${expiry}:${sig}`;
...
export function verifySessionToken(sessionSecret, token) {
  ...
  try {
    ...
    if (parts.length === 3) {
      ...
      return { handle, expiry, packId: null };
    } else if (parts.length === 4) {
      const [handle, packId, expiryStr, sig] = parts;
      const expiry = parseInt(expiryStr, 10);
      if (Date.now() > expiry) return null;
      const expectedSig = hmacSha256(sessionSecret, `${handle}:${packId}:${expiry}`);
      if (!timingSafeEqualStr(sig, expectedSig)) return null;
